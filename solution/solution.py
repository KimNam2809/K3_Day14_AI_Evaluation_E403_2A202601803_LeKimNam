...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        for chunk in contexts:
            ...
            is_relevant = (overlap / len(expected_tokens)) >= relevance_threshold if expected_tokens else True
            relevant_chunks.append(is_relevant)
...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    return sorted(contexts, key=lambda c: len(_tokenize(c).intersection(query_tokens)), reverse=True)
...
class LLMJudge:
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for i, item in enumerate(scores_batch):
            ...
            if scores:
                avg_for_item = sum(scores.values()) / len(scores)
                total_score += avg_for_item
                count += 1
                if i == 0:
                    first_score_avg = avg_for_item
        ...
        positional = False
        if count > 1 and first_score_avg > overall_avg:
             positional = True
...
class BenchmarkRunner:
    ...
    def generate_report(self, results: list[EvalResult]) -> dict[str, Any]:
        ...
        c_sum = sum(r.completeness for r in results)
        ...
        return {
            ...
            "avg_faithfulness": f_sum / total,
            "avg_relevance": r_sum / total,
            "avg_completeness": c_sum / total,
            "avg_context_recall": sum(recall_vals) / len(recall_vals) if recall_vals else None,
            "avg_context_precision": sum(prec_vals) / len(prec_vals) if prec_vals else None,
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        """
        new_report = self.generate_report(new_results) if new_results else {}
        base_report = self.generate_report(baseline_results) if baseline_results else {}
        
        regressions = []
        metrics = ['avg_faithfulness', 'avg_relevance', 'avg_completeness']
        
        for m in metrics:
            new_val = new_report.get(m, 0.0)
            base_val = base_report.get(m, 0.0)
        ...
        return {
            ...
            "baseline_avg_faithfulness": base_report.get("avg_faithfulness", 0.0),
            "baseline_avg_relevance": base_report.get("avg_relevance", 0.0),
            "baseline_avg_completeness": base_report.get("avg_completeness", 0.0),
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        metrics = [
            (failure.faithfulness, "Context is missing or irrelevant — improve retrieval"),
            (failure.relevance, "Answer does not address the question — improve prompt clarity"),
            (failure.completeness, "Answer is missing key information — increase context window or improve generation")
        ]
        
        mins = [m for m in metrics if m[0] == min_score]
        if len(mins) > 1:
            return "Multiple issues detected — review full pipeline"
            
        if mins[0][0] == failure.faithfulness:
            return "Context is missing or irrelevant — improve retrieval"
        elif mins[0][0] == failure.relevance:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for i, f in enumerate(failures):
            # Accessing QA pair id if it exists in metadata or attribute
            fid = f"F{i+1:03d}"
            if hasattr(f, 'qa_pair') and f.qa_pair:
                if hasattr(f.qa_pair, 'id'):
                    fid = f.qa_pair.id
                elif isinstance(f.qa_pair.metadata, dict) and 'id' in f.qa_pair.metadata:
                    fid = f.qa_pair.metadata['id']
    ...
    def generate_improvement_suggestions(
        ...
        if "irrelevant" in cats:
            suggestions.append("Improve prompt clarity to address user intent")
        if "off_topic" in cats:
            suggestions.append("Enhance intent detection to filter out off-topic requests")
            
        while len(suggestions) < 3:
            suggestions.append("General pipeline review and tuning")
            
        # For generate_improvement_log, it expects one suggestion per failure if we pass suggestions.
        # But this function just returns a general prioritized list. We'll return max(3, len(failures))
        while len(suggestions) < len(failures):
            suggestions.append("Review failure and tune model/pipeline")
